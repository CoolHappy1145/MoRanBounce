...
/* loaded from: L-out.jar:net/ccbluex/liquidbounce/ui/client/clickgui/style/styles/LiquidBounceStyle.class */
public class LiquidBounceStyle extends Style {
    ...
    public void drawPanel(int i, int i2, Panel panel) {
        ...
        Fonts.font35.drawString("\u00a7f" + panel.getName(), (int) (panel.getX() - ((Fonts.font35.getStringWidth("\u00a7f" + StringUtils.func_76338_a(panel.getName())) - 100.0f) / 2.0f)), panel.getY() + 7, -16777216);
        if (panel.getScrollbar() && panel.getFade() > 0) {
            ...
            RenderUtils.drawRect(panel.getX() - 2, ((panel.getY() + 30) + (((panel.getFade() - 24.0f) / (panel.getElements().size() - ((Integer) ((ClickGUI) LiquidBounce.moduleManager.getModule(ClickGUI.class)).maxElementsValue.get()).intValue())) * panel.getDragged())) - 10.0f, panel.getX(), panel.getY() + 40 + (((panel.getFade() - 24.0f) / (panel.getElements().size() - ((Integer) ((ClickGUI) LiquidBounce.moduleManager.getModule(ClickGUI.class)).maxElementsValue.get()).intValue())) * panel.getDragged()), Integer.MIN_VALUE);
        }
    }

    @Override // net.ccbluex.liquidbounce.p005ui.client.clickgui.style.Style
    public void drawDescription(int i, int i2, String str) {
        RenderUtils.drawBorderedRect(i + 9, i2, i + Fonts.font35.getStringWidth(str) + 14, i2 + Fonts.font35.getFontHeight() + 3, 1.0f, new Color(255, 255, 255, 90).getRGB(), Integer.MIN_VALUE);
                ...
                for (Value value : values) {
                    ...
                    if (value instanceof BoolValue) {
                        ...
                        for (String str : listValue.getValues()) {
                            ...
                            if (moduleElement.getSettingsWidth() < stringWidth3 + 8.0f) {
                                moduleElement.setSettingsWidth(stringWidth3 + 8.0f);
                            }
