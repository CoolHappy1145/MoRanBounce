package net.ccbluex.liquidbounce.injection.backend;

import kotlin.Metadata;
import kotlin.jvm.internal.Intrinsics;
import net.ccbluex.liquidbounce.LiquidBounce;
import net.ccbluex.liquidbounce.api.minecraft.tileentity.ITileEntity;
import net.ccbluex.liquidbounce.api.minecraft.util.WBlockPos;
import net.minecraft.tileentity.TileEntity;
import net.minecraft.util.math.BlockPos;
import org.jetbrains.annotations.NotNull;
import org.jetbrains.annotations.Nullable;

@Metadata(m24mv = {1, 1, 16}, m25bv = {1, 0, 3}, m23k = 1, m26d1 = {"\ufffd\ufffd&\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\ufffd\ufffd\n\u0002\u0018\u0002\n\u0002\b\u0002\n\u0002\u0018\u0002\n\u0002\b\u0005\n\u0002\u0010\u000b\n\ufffd\ufffd\n\u0002\u0010\ufffd\ufffd\n\ufffd\ufffd\u0018\ufffd\ufffd2\u00020\u0001B\r\u0012\u0006\u0010\u0002\u001a\u00020\u0003\u00a2\u0006\u0002\u0010\u0004J\u0013\u0010\u000b\u001a\u00020\f2\b\u0010\r\u001a\u0004\u0018\u00010\u000eH\u0096\u0002R\u0014\u0010\u0005\u001a\u00020\u00068VX\u0096\u0004\u00a2\u0006\u0006\u001a\u0004\b\u0007\u0010\bR\u0011\u0010\u0002\u001a\u00020\u0003\u00a2\u0006\b\n\ufffd\ufffd\u001a\u0004\b\t\u0010\n\u00a8\u0006\u000f"}, m27d2 = {"Lnet/ccbluex/liquidbounce/injection/backend/TileEntityImpl;", "Lnet/ccbluex/liquidbounce/api/minecraft/tileentity/ITileEntity;", "wrapped", "Lnet/minecraft/tileentity/TileEntity;", "(Lnet/minecraft/tileentity/TileEntity;)V", "pos", "Lnet/ccbluex/liquidbounce/api/minecraft/util/WBlockPos;", "getPos", "()Lnet/ccbluex/liquidbounce/api/minecraft/util/WBlockPos;", "getWrapped", "()Lnet/minecraft/tileentity/TileEntity;", "equals", "", "other", "", LiquidBounce.CLIENT_NAME})
/* loaded from: L-out.jar:net/ccbluex/liquidbounce/injection/backend/TileEntityImpl.class */
public final class TileEntityImpl implements ITileEntity {

    @NotNull
    private final TileEntity wrapped;

    @NotNull
    public final TileEntity getWrapped() {
        return this.wrapped;
    }

    public TileEntityImpl(@NotNull TileEntity wrapped) {
        Intrinsics.checkParameterIsNotNull(wrapped, "wrapped");
        this.wrapped = wrapped;
    }

    @Override // net.ccbluex.liquidbounce.api.minecraft.tileentity.ITileEntity
    @NotNull
    public WBlockPos getPos() {
        BlockPos blockPosFunc_174877_v = this.wrapped.func_174877_v();
        Intrinsics.checkExpressionValueIsNotNull(blockPosFunc_174877_v, "wrapped.pos");
        return new WBlockPos(blockPosFunc_174877_v.func_177958_n(), blockPosFunc_174877_v.func_177956_o(), blockPosFunc_174877_v.func_177952_p());
    }

    public boolean equals(@Nullable Object obj) {
        return (obj instanceof TileEntityImpl) && Intrinsics.areEqual(((TileEntityImpl) obj).wrapped, this.wrapped);
    }
}
