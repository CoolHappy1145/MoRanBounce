...
/* loaded from: L-out.jar:org/spongepowered/tools/obfuscation/AnnotatedMixinElementHandlerAccessor.class */
public class AnnotatedMixinElementHandlerAccessor extends AnnotatedMixinElementHandler implements IMixinContext {

    /* loaded from: L-out.jar:org/spongepowered/tools/obfuscation/AnnotatedMixinElementHandlerAccessor$AnnotatedElementAccessor.class */
    static class AnnotatedElementAccessor extends AnnotatedMixinElementHandler.AnnotatedElement {
        ...
        public TypeMirror getTargetType() {
            switch (C05771.f229xd539c0f6[getAccessorType().ordinal()]) {
                case 1:
    ...
    /* renamed from: org.spongepowered.tools.obfuscation.AnnotatedMixinElementHandlerAccessor$1 */
    /* loaded from: L-out.jar:org/spongepowered/tools/obfuscation/AnnotatedMixinElementHandlerAccessor$1.class */
    static /* synthetic */ class C05771 {

        /* renamed from: $SwitchMap$org$spongepowered$asm$mixin$gen$AccessorInfo$AccessorType */
        static final int[] f229xd539c0f6 = new int[AccessorInfo.AccessorType.values().length];

        static {
            try {
                f229xd539c0f6[AccessorInfo.AccessorType.FIELD_GETTER.ordinal()] = 1;
            } catch (NoSuchFieldError unused) {
            }
            try {
                f229xd539c0f6[AccessorInfo.AccessorType.FIELD_SETTER.ordinal()] = 2;
            } catch (NoSuchFieldError unused2) {
            }
        }
    }

    /* loaded from: L-out.jar:org/spongepowered/tools/obfuscation/AnnotatedMixinElementHandlerAccessor$AnnotatedElementInvoker.class */
    static class AnnotatedElementInvoker extends AnnotatedElementAccessor {
        ...
        public void attach(TypeHandle typeHandle) {
            ...
            AccessorInfo.AccessorName accessorNameM55of = AccessorInfo.AccessorName.m55of(getSimpleName(), false);
            if (accessorNameM55of == null) {
                return;
            }
            Iterator it = AccessorInfo.AccessorType.OBJECT_FACTORY.getExpectedPrefixes().iterator();
            while (it.hasNext()) {
                if (((String) it.next()).equals(accessorNameM55of.prefix) && (Constants.CTOR.equals(accessorNameM55of.name) || typeHandle.getSimpleName().equals(accessorNameM55of.name))) {
                    this.type = AccessorInfo.AccessorType.OBJECT_FACTORY;
                    return;
                }
            }
    ...
    public void registerAccessor(AnnotatedElementAccessor elem) {
        if (elem.getAccessorType() == null) {
            elem.printMessage(this.f228ap, Diagnostic.Kind.WARNING, "Unsupported accessor type");
            return;
        }
        String targetName = getAccessorTargetName(elem);
        if (targetName == null) {
            elem.printMessage(this.f228ap, Diagnostic.Kind.WARNING, "Cannot inflect accessor target name");
            return;
        }
    ...
    private void registerAccessorForTarget(AnnotatedElementAccessor elem, TypeHandle target) {
        ...
        if (targetField == null) {
            if (!target.isImaginary()) {
                elem.printMessage(this.f228ap, Diagnostic.Kind.ERROR, "Could not locate @Accessor target " + elem + " in target " + target);
                return;
            }
        ...
        if (obfData.isEmpty()) {
            ...
            elem.printMessage(this.f228ap, Diagnostic.Kind.WARNING, "Unable to locate obfuscation mapping" + info + " for @Accessor target " + elem);
        } else {
            try {
                ...
                elem.printMessage(this.f228ap, Diagnostic.Kind.ERROR, "Mapping conflict for @Accessor target " + elem + ": " + ex.getNew() + " for target " + target + " conflicts with existing mapping " + ex.getOld());
            }
        }
    }
    ...
    private void registerInvokerForTarget(AnnotatedElementInvoker elem, TypeHandle target) {
        ...
        if (targetMethod == null) {
            if (!target.isImaginary()) {
                elem.printMessage(this.f228ap, Diagnostic.Kind.ERROR, "Could not locate @Invoker target " + elem + " in target " + target);
                return;
            }
        ...
        if (obfData.isEmpty()) {
            ...
            try {
                ...
                elem.printMessage(this.f228ap, Diagnostic.Kind.ERROR, "Mapping conflict for @Invoker target " + elem + ": " + ex.getNew() + " for target " + target + " conflicts with existing mapping " + ex.getOld());
            }
        }
    }

    private void registerFactoryForTarget(AnnotatedElementInvoker elem, TypeHandle target) {
        TypeUtils.EquivalencyResult equivalency = TypeUtils.isEquivalentType(this.f228ap.getProcessingEnvironment(), elem.getReturnType(), target.getType());
        if (equivalency.type != TypeUtils.Equivalency.EQUIVALENT) {
            if (equivalency.type == TypeUtils.Equivalency.EQUIVALENT_BUT_RAW && equivalency.rawType == 1) {
                elem.printMessage(this.f228ap, Diagnostic.Kind.WARNING, "Raw return type for Factory @Invoker", SuppressedBy.RAW_TYPES);
            } else if (equivalency.type == TypeUtils.Equivalency.BOUNDS_MISMATCH) {
                elem.printMessage(this.f228ap, Diagnostic.Kind.ERROR, "Invalid Factory @Invoker return type, generic type arguments of " + target.getType() + " are incompatible with " + elem.getReturnType() + ". " + equivalency);
                return;
            } else {
                elem.printMessage(this.f228ap, Diagnostic.Kind.ERROR, "Invalid Factory @Invoker return type, expected " + target.getType() + " but found " + elem.getReturnType());
                return;
            }
        }
        if (!elem.isStatic()) {
            elem.printMessage(this.f228ap, Diagnostic.Kind.ERROR, "Factory @Invoker must be static");
        } else {
            if (!elem.shouldRemap()) {
                return;
            }
