...
/* loaded from: L-out.jar:jdk/nashorn/internal/runtime/FindProperty.class */
public final class FindProperty {
    ...
    public MethodHandle getGetter(Class<?> type, int programPoint, LinkRequest request) {
        ...
        if (this.property instanceof UserAccessorProperty) {
            MethodHandle getter2 = Lookup.f31MH.insertArguments(getter, 1, new Object[]{UserAccessorProperty.getINVOKE_UA_GETTER(type, programPoint)});
            if (UnwarrantedOptimismException.isValid(programPoint) && type.isPrimitive()) {
                getter2 = Lookup.f31MH.insertArguments(getter2, 1, new Object[]{Integer.valueOf(programPoint)});
            }
    ...
    private MethodHandle insertAccessorsGetter(UserAccessorProperty uap, LinkRequest request, MethodHandle mh) {
        ...
        if (request != null && !(request.getReceiver() instanceof ScriptObject)) {
            ...
            superGetter = Lookup.f31MH.filterArguments(superGetter, 0, new MethodHandle[]{wrapFilter.asType(wrapFilter.type().changeReturnType(superGetter.type().parameterType(0)))});
        }
        return Lookup.f31MH.foldArguments(mh, Lookup.f31MH.asType(superGetter, superGetter.type().changeParameterType(0, Object.class)));
    }
