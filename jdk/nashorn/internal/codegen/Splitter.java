...
/* loaded from: L-out.jar:jdk/nashorn/internal/codegen/Splitter.class */
final class Splitter extends SimpleNodeVisitor implements Loggable {
    ...
    FunctionNode split(FunctionNode fn, boolean top) {
        ...
        this.log.fine(new Object[]{"Initiating split of '", functionNode2.getName(), "'"});
        long weight = WeighNodes.weigh(functionNode2);
        if (!$assertionsDisabled && !this.f30lc.isEmpty()) {
            throw new AssertionError("LexicalContext not empty");
        }
        if (weight >= SPLIT_THRESHOLD) {
            this.log.info(new Object[]{"Splitting '", functionNode2.getName(), "' as its weight ", Long.valueOf(weight), " exceeds split threshold ", Long.valueOf(SPLIT_THRESHOLD)});
        ...
        List<FunctionNode> dc = directChildren(functionNode);
        Block newBody = (Block) body.accept(new SimpleNodeVisitor(this, dc) { // from class: jdk.nashorn.internal.codegen.Splitter.1
            final List val$dc;
            ...
            public boolean enterFunctionNode(FunctionNode functionNode3) {
                return this.val$dc.contains(functionNode3);
            }

            @Override // jdk.nashorn.internal.p001ir.visitor.NodeVisitor
            public Node leaveFunctionNode(FunctionNode functionNode3) {
                FunctionNode functionNodeSplit = new Splitter(this.this$0.compiler, functionNode3, this.this$0.outermostCompileUnit).split(functionNode3, false);
                this.f30lc.replace(functionNode3, functionNodeSplit);
                return functionNodeSplit;
            }
        });
    ...
    private static List<FunctionNode> directChildren(FunctionNode functionNode) {
        List<FunctionNode> dc = new ArrayList<>();
        functionNode.accept(new SimpleNodeVisitor(functionNode, dc) { // from class: jdk.nashorn.internal.codegen.Splitter.2
            ...
            public boolean enterFunctionNode(FunctionNode functionNode2) {
                if (functionNode2 == this.val$functionNode) {
                    return true;
                }
                if (this.f30lc.getParentFunction(functionNode2) == this.val$functionNode) {
                    this.val$dc.add(functionNode2);
                    return false;
                }
                return false;
            }
    ...
    private Block splitBlock(Block block, FunctionNode function) {
        ...
        return block.setStatements(this.f30lc, splits);
    }
    ...
    public Node leaveBlock(Block block) {
        ...
        if (weight >= SPLIT_THRESHOLD) {
            ...
            this.f30lc.setFlag(currentFunction, 16);
        }
    ...
    public Node leaveLiteralNode(LiteralNode literal) {
        ...
        this.f30lc.setFlag(functionNode, 16);
        if (literal instanceof LiteralNode.ArrayLiteralNode) {
            ...
            return arrayLiteralNode.setSplitRanges(this.f30lc, ranges);
        }
        return literal;
    }
    ...
    public Node leaveObjectNode(ObjectNode objectNode) {
        ...
        return objectNode.setSplitRanges(this.f30lc, ranges);
    }
