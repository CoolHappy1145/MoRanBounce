...
/* loaded from: L-out.jar:jdk/nashorn/internal/codegen/Label.class */
public final class Label implements Serializable {
    ...
    /* loaded from: L-out.jar:jdk/nashorn/internal/codegen/Label$Stack.class */
    static final class Stack implements Cloneable {
        ...
        void push(Type type) {
            if (this.data.length == this.f15sp) {
                Type[] newData = new Type[this.f15sp * 2];
                int[] newLocalLoad = new int[this.f15sp * 2];
                System.arraycopy(this.data, 0, newData, 0, this.f15sp);
                System.arraycopy(this.localLoads, 0, newLocalLoad, 0, this.f15sp);
                this.data = newData;
                this.localLoads = newLocalLoad;
            }
            this.data[this.f15sp] = type;
            this.localLoads[this.f15sp] = -1;
            this.f15sp++;
        }
        ...
        Type[] getTopTypes(int count) {
            ...
            System.arraycopy(this.data, this.f15sp - count, topTypes, 0, count);
            return topTypes;
        }
        ...
        Type pop() {
            if (!$assertionsDisabled && this.f15sp <= 0) {
                throw new AssertionError();
            }
        ...
        private Stack cloneWithEmptyStack() {
            Stack stack = m179clone();
            stack.f15sp = 0;
            return stack;
        }

        int getTopLocalLoad() {
            return this.localLoads[this.f15sp - 1];
        }

        void markLocalLoad(int slot) {
            this.localLoads[this.f15sp - 1] = slot;
        }
        ...
        public String toString() {
            return "stack=" + Arrays.toString(Arrays.copyOf(this.data, this.f15sp)) + ", symbolBoundaries=" + String.valueOf(this.symbolBoundary) + ", firstTemp=" + this.firstTemp + ", localTypes=" + String.valueOf(this.localVariableTypes);
        }
    }
    ...
    void joinFrom(Stack stack) {
        this.reachable = true;
        if (this.stack == null) {
            this.stack = stack.m179clone();
        } else {
            this.stack.joinFrom(stack, this.breakTarget);
        }
    }

    void joinFromTry(Stack stack, boolean z) {
        this.reachable = true;
        if (this.stack == null) {
            if (!z) {
    ...
    void markAsOptimisticCatchHandler(Stack stack, int i) {
        this.stack = stack.cloneWithEmptyStack();
        this.stack.markAsOptimisticCatchHandler(i);
    }

    void markAsOptimisticContinuationHandlerFor(Label label) {
    ...
    boolean isAfter(Label label) {
        return this.label.getOffset() > label.label.getOffset();
    }

    public String toString() {
        if (this.str == null) {
            this.str = this.name + '_' + this.f14id;
        }
        return this.str;
    }
}
