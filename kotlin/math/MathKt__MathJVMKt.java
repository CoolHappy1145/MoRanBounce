...
class MathKt__MathJVMKt extends MathKt__MathHKt {
    ...
    public static final double log(double x, double base) {
        return (base <= 0.0d || base == 1.0d) ? DoubleCompanionObject.INSTANCE.getNaN() : Math.log(x) / Math.log(base);
    }

    @SinceKotlin(version = "1.2")
    @InlineOnly
    /* renamed from: ln */
    private static final double m41ln(double x) {
        return Math.log(x);
    }
    ...
    public static final float log(float x, float base) {
        return (base <= 0.0f || base == 1.0f) ? FloatCompanionObject.INSTANCE.getNaN() : (float) (Math.log(x) / Math.log(base));
    }

    @SinceKotlin(version = "1.2")
    @InlineOnly
    /* renamed from: ln */
    private static final float m42ln(float x) {
